...
class _WhatsAppVoiceNoteState extends State<WhatsAppVoiceNote> {
      ...
      child: Container(
        decoration: BoxDecoration(
          color: widget.isSentByMe 
              ? (Theme.of(context).brightness == Brightness.dark ? const Color(0xFF054740) : const Color(0xFFDCF8C6))
              : (Theme.of(context).brightness == Brightness.dark ? const Color(0xFF1F2C33) : Colors.white),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: widget.isSentByMe 
                ? Colors.transparent 
                : (Theme.of(context).brightness == Brightness.dark ? Colors.white.withOpacity(0.1) : Colors.grey[300]!),
            width: 1,
          ),
          boxShadow: [
            BoxShadow(
        ...
        child: Padding(
          ...
          child: AudioPlayerWidget(
            source: widget.voice.uploadUrl ?? widget.voice.url,
            durationMs: widget.voice.durationMs,
            waveformData: (widget.voice.waveformData?.isNotEmpty ?? false) 
  ...
  Widget _buildWaveform() {
    try {
      if (widget.voice.waveformData?.isEmpty ?? true) {
