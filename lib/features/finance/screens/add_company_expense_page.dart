...
class AddCompanyExpensePage extends StatefulWidget {
  final Map<String, dynamic>? expense;

  const AddCompanyExpensePage({super.key, this.expense});

  @override
  State<AddCompanyExpensePage> createState() => _AddCompanyExpensePageState();
}

class _AddCompanyExpensePageState extends State<AddCompanyExpensePage> {
  ...
  Future<void> _saveExpense() async {
    ...
    try {
      ...
      if (_isEditing) {
        result = await ExpenseService.updateCompanyExpense(
          widget.expense!['id'],
          expenseData,
        );
      } else {
        result = await ExpenseService.createCompanyExpense(expenseData);
      }
  ...
  Widget build(BuildContext context) {
    ...
    return ProtectedRoute(
      requiredPermissions: ['manage_company_expenses'],
      child: Scaffold(
        backgroundColor: Colors.transparent,
        extendBodyBehindAppBar: true,
        appBar: LuxuryAppBar(
          title: _isEditing ? 'Modifier la Dépense Société' : 'Ajouter une Dépense Société',
  ...
  Widget _buildTitleSection(bool isDark, SettingsController settings) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Icon(
            _isEditing ? Icons.edit_rounded : Icons.business_rounded,
            size: 48,
            color: AppTheme.accentGold,
          ),
          const SizedBox(height: 16),
          Text(
            _isEditing ? 'Modifier la Dépense Société' : 'Ajouter une Dépense Société',
            style: TextStyle(
              fontSize: 24,
          ...
          Text(
            _isEditing 
                ? 'Modifiez les informations de la dépense de la société'
                : 'Remplissez les informations pour ajouter une nouvelle dépense de la société',
            style: TextStyle(
              fontSize: 14,
