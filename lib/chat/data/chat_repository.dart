...
class ChatRepository {
  ...
  Future<List<ChatMessage>> getMessages({
    ...
    try {
      ...
      print('[REPO] ERROR getting messages: $e');
  ...
  Future<ChatMessage> sendMessageWithAttachments({
    ...
    if (_isOnline) {
      try {
        ...
        // POST to server with attachments
        print('[REPO] Posting message with attachments to REST endpoint...');
        final response = await rest.sendMessageWithAttachments(
          conversationId: conversationId,
          senderId: senderId,
          content: content,
          type: type,
        ...
        if (response['success']) {
          ...
          _idempotencyMap[clientId] = serverId;
        ...
        print('[REPO] Error sending message with attachments: $e');
        // Transition: sending → failed → queued
        var msg = message.copyWith(
          state: MessageState.queued,
          retryCount: 1,
      ...
      print('[REPO] Offline: queuing message with attachments');
      // Transition: draft → queued
      var msg = message.copyWith(state: MessageState.queued);
      store[clientId] = msg;
      _messageUpdated.add(msg);
