...
class ChatService {
  ...
  static Future<Map<String, dynamic>> getConversations({
    ...
    try {
      ...
      for (final r in res.rows) {
        final convType = r.colByName('type');
        final convIdRaw = r.colByName('id');
        final convId = convIdRaw is int ? convIdRaw : int.tryParse(convIdRaw?.toString() ?? '');
        ...
        if (convType == 'direct' && convId != null) {
          try {
            final memberRes = await conn.execute(
            ...
            if (memberRes.rows.isNotEmpty) {
              ...
              if (otherUserId != null) {
                ...
                // Fetch employee details: prenom, nom, photo
                final empRes = await conn.execute(
                ...
                if (empRes.rows.isNotEmpty) {
                  ...
                  // Build full name from prenom and nom
                  ...
                  // Build full media URL for photo
                  if (photo.isNotEmpty) {
                    // For base64 images, create a data URL directly instead of using media endpoint
                    if (photo.startsWith('data:image') || photo.startsWith('iVBORw0K') || (photo.length > 100 && !photo.contains('/') && !photo.contains('\\'))) {
                      displayAvatarUrl = photo; // Use base64 data URL directly
                      print('📸 Photo found as base64 data URL');
                    } else {
                      final cleanPhoto = photo.replaceAll(RegExp(r'^[/\\]+'), '');
                      displayAvatarUrl = 'http://localhost:8080/media/$cleanPhoto';
                      print('📸 Photo found as file: raw=$photo, clean=$cleanPhoto, final URL=$displayAvatarUrl');
                    }
                  } else {
                    print('⚠️ No photo for user $otherUserId');
                  }
                  
                  print('✅ Employee loaded for conv $convId: $displayName, avatar=$displayAvatarUrl');
                }
              }
            }
          } catch (e) {
            print('⚠️ Error loading employee for conv $convId: $e');
          }
        }
        
        // Fallback name if not set
        displayName ??= receiverId ?? 'Contact';
        
        conversations.add({
          'id': convIdRaw?.toString(),
          'name': displayName,
          'type': convType,
        ...
        print('📤 Conversation response: name=$displayName, avatarUrl=$displayAvatarUrl, type=$convType');
      }
  ...
  static Future<Map<String, dynamic>> getMessages({
    ...
    try {
      ...
      print('[CHAT-SVC] Fetching messages for conv: $conversationId, query: $query');
      final res = await conn.execute(query, params);
      final rows = res.rows.toList(); // Materialize rows immediately
      if (rows.isEmpty) {
      ...
      final messageIds = rows
          ...
          .where((id) => (id as String).isNotEmpty)
          .toList();
      final idList = messageIds.map((id) => "'$id'").join(',');

      // Batch fetch ALL attachments for all messages in ONE query
      final attRes = await conn.execute('''
        SELECT message_id, id, filename, original_filename, file_path, file_size, mime_type, thumbnail_path, created_at
        FROM message_attachments
        WHERE message_id IN ($idList)
      ''');
      final attRows = attRes.rows.toList(); // Materialize rows immediately
      ...
      for (final r in attRows) {
        ...
        attachmentsByMessage.putIfAbsent(msgId, () => []).add({
          'id': r.colByName('id')?.toString(),
          'type': type,
          'filename': r.colByName('original_filename') ?? storedFilename,
          'mimeType': mimeType,
          'file_size': sizeVal,
          'media_url': storedFilename != null ? 'http://localhost:8080/media/$storedFilename' : '',
          'thumbnail_url': thumbnailStored != null ? 'http://localhost:8080/media/$thumbnailStored' : null,
          'displaySize': displaySize,
        });
      }

      // Batch fetch ALL voice messages for all messages in ONE query
      final vmRes = await conn.execute('''
        SELECT message_id, id, file_path, duration_seconds, waveform_data, file_size, created_at
        FROM voice_messages
        WHERE message_id IN ($idList)
      ''');
      final vmRows = vmRes.rows.toList(); // Materialize rows immediately

      // Group voice by message_id
      final voiceByMessage = <String, Map<String, dynamic>>{};
      for (final v in vmRows) {
        ...
        final storedVoice = v.colByName('file_path')?.toString();
        String voiceMediaUrl = '';
        if (storedVoice != null) {
          final parts = storedVoice.split('/');
          final candidate = parts.isNotEmpty ? parts.last : storedVoice;
          voiceMediaUrl = 'http://localhost:8080/media/$candidate';
        }

        voiceByMessage[msgId] = {
          'fileId': v.colByName('id')?.toString(),
          'duration_seconds': int.tryParse(v.colByName('duration_seconds')?.toString() ?? '0') ?? 0,
          'waveform': waveform,
          'transcription': null,
          'media_url': voiceMediaUrl,
        };
      ...
      // Assemble messages using in-memory maps
      final messages = rows.map((row) {
        final msgId = row.colByName('id')?.toString() ?? '';
        return {
          'id': msgId,
          'conversationId': row.colByName('conversation_id')?.toString(),
          'senderId': row.colByName('sender_id')?.toString(),
          ...
          'voiceMessage': voiceByMessage[msgId],
          'reactions': [],
          'isRead': false,
        };
      }).toList();
  ...
  static Future<Map<String, dynamic>> sendMessage({
    ...
    try {
      ...
      final msgFinal = (clientMessageId != null && clientMessageId.isNotEmpty)
          ? await conn.transaction<Map<String, dynamic>>((tx) async {
              // membership
          ...
          : await _sendNonIdempotent(conn, conversationId, senderId, content, type, replyToId, uploadIds, voiceMetadata);

      if (msgFinal['success'] == true) {
        final built = msgFinal['message'];
        WebSocketServer.broadcastToConversationMembers(
          conversationId,
          {
            'type': 'message:created',
            'payload': {'message': built},
            'timestamp': DateTime.now().millisecondsSinceEpoch,
          },
          excludeUserId: senderId.toString(),
        );
      }
  ...
  static Future<Map<String, dynamic>> _sendNonIdempotent(dynamic conn, String conversationId, String senderId, String content, String type, String? replyToId, List<String>? uploadIds, Map<String, dynamic>? voiceMetadata) async {
  ...
  static Future<Map<String, dynamic>> leaveConversation({required String conversationId, required String userId}) async {
    try {
      ...
      final convRes = await conn.execute('SELECT type FROM conversations WHERE id = :id', {'id': conversationId});
      if (convRes.rows.isEmpty) return {'success': false, 'message': 'Conversation not found'};
      final type = convRes.rows.first.colByName('type');

      // Mark as left
      await conn.execute(
        'UPDATE conversation_members SET left_at = NOW() WHERE conversation_id = :conversationId AND user_id = :userId',
        {'conversationId': conversationId, 'userId': userId}
      );

      // If it's a group, optionally send a system message
      if (type == 'group') {
        // Fetch username for system message
        final userRes = await conn.execute('SELECT username FROM users WHERE id = :userId', {'userId': userId});
        final username = userRes.rows.isNotEmpty ? userRes.rows.first.colByName('username') : 'A user';
    ...
    try {
      ...
      if (type == 'direct' && allParticipants.length == 2) {
        ...
        final existing = await conn.execute('''
          ...
          WHERE c.type = 'direct' 
          AND cm1.user_id = :user1Id 
          AND cm2.user_id = :user2Id
          AND cm1.left_at IS NULL
          AND cm2.left_at IS NULL
        ''', {'user1Id': user1Id, 'user2Id': user2Id});
        
        if (existing.rows.isNotEmpty) {
          final convId = existing.rows.first.colByName('id');
          return {
            'success': true, 
            'message': 'Conversation already exists', 
            'data': {'conversationId': convId.toString()}
          };
        }
      }
      ...
      if (conversationId == null) {
        return {'success': false, 'message': 'Failed to create conversation record'};
      }
      ...
      // If group chat, create a system message
      if (type == 'group') {
        await sendMessage(
      ...
      print('Error in ChatService.createConversation: $e');
